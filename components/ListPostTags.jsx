...
export default async function ListPostTags({ tags }) {
  ...
  return (
    ...
    <section className="w-full px-4 py-4 sm:h-screen ">
      ...
      <div className="grid grid-cols-1  sm:grid-cols-3 md:grid-cols-3 gap-8 px-4">
        {posts.map(post => (
          <article className="w-full max-w-94 mx-auto border-b-2 mt-8 w-80  overflow-hidden" key={post.id}>
